        ...
        <div class="w-full flex flex-col gap-0 bg-slate-50 border-y border-slate-100">
                        ...
                        <div class="w-full flex items-center gap-5 text-slate-500 text-xs">
                            @if(
                                ...
                                <button type="button" class="flex items-center gap-2 hover:cursor-pointer">
                                    ...
                                    <span wire:click="selectComment({{ $c->id }})">{{ $c->likes->count() }} {{ trans_choice('forumium.likes', $c->likes->count()) }}</span>
                                </button>
                            @else
                                <div class="flex items-center gap-2">
                                    <i class="@if($c->likes()->where('user_id', auth()->user()->id)->count()) fa-solid text-red-500 @else fa-regular @endif fa-thumbs-up"></i> {{ $c->likes->count() }} {{ trans_choice('forumium.likes', $c->likes->count()) }}
                                </div>
                            @endif
                            @if(
                                (auth()->user() && auth()->user()->hasVerifiedEmail()) &&
            ...
            @else
                <span class="text-slate-700 font-medium text-sm py-5 px-3">
                    {{ trans('forumium.no_comments_yet_please_come_back_later_or_add_one') }}
                </span>
            @endif
        </div>
        @if($comment)
