            ...
            <div class="flex items-center gap-1">
                ...
                @switch($type)
                    @case(Followers::FOLLOWING->value)
                        <i class="fa-solid fa-star text-green-500" title="{{ trans('forumium.following') }}"></i>
                        @break
                    @case(Followers::NOT_FOLLOWING->value)
                        <i class="fa-regular fa-star text-orange-500" title="{{ trans('forumium.not_following') }}"></i>
                        @break
                    @case(Followers::IGNORING->value)
                        <i class="fa-regular fa-eye-slash text-red-500" title="{{ trans('forumium.ignoring') }}"></i>
                        @break
                @endswitch
                <span class="font-medium text-slate-500">
                    @if($discussion->is_resolved)
                        <span class="font-normal">[{{ trans('forumium.resolved') }}]</span>
                    @endif
                    {{ $discussion->name }}
                </span>
            </div>
            <span class="text-slate-400 text-sm">
                {{ trans('forumium.created_by') }} <span class="font-medium">{{ $discussion->user->name }}</span> (<span class="text-xs">{{ $discussion->created_at->diffForHumans() }}</span>)
            </span>
