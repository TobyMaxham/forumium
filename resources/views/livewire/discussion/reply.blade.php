...
<div class="p-6 space-y-6">
    <h3 class="text-xl font-medium text-slate-900 dark:text-white">{{ $reply?->id ? trans('forumium.update_reply_content') : trans('forumium.post_a_reply') }}</h3>
    <form>
        {{ $this->form }}
        <!-- Modal footer -->
        <div class="flex items-center space-x-2 rounded-b mt-5">
            <button wire:click="submit" type="button" wire:loading.attr="disabled"
                ...
                {{ $reply?->id ? trans('forumium.edit_reply') : trans('forumium.add_reply') }}
            </button>
            @if($reply?->id)
