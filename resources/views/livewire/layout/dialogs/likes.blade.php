<div class="px-6 py-6 lg:px-8 overflow-auto max-h-[90vh]">
    <h3 class="mb-8 text-xl font-medium text-slate-900 dark:text-white">{{ trans_choice('forumium.likes', 2) }} @if($likes->count()) ({{ $likes->count() }}) @endif</h3>
    <div class="w-full flex flex-col">
        @if($likes->count())
            @foreach($likes as $like)
                <div class="w-full flex items-center gap-2 py-3 @if(!$loop->last) border-b border-slate-200 @endif">
                    <img src="{{ $like->user->avatarUrl }}" alt="{{ trans('forumium.avatar') }}"
                    ...
                    <div class="flex flex-col gap-0">
                        ...
                        <span class="text-slate-500 text-sm">{{ trans('forumium.liked') }} {{ $like->created_at->diffForHumans() }}</span>
                    </div>
                </div>
            @endforeach
        @else
            <span class="text-slate-700 font-medium text-sm">
            {{ trans('forumium.no_likes_available_yet') }}
        </span>
        @endif
    </div>
</div>
